#!/usr/bin/env python3
"""Open Plan Route sheet and take screenshot."""
import asyncio
import json
import subprocess
import websockets
    ...
    async with websockets.connect(VM_URL) as ws:
        ...
        async def evaluate(expr, wait=0.5):
            ...
            if "error" in resp:
                ...
                print(f"  OK: {result.get('kind')}")
            await asyncio.sleep(wait)
            return resp

        # Open Plan Route sheet (goes to map tab + opens sheet)
        print("1. Opening Plan Route sheet...")
        await evaluate("openPlanRouteSheet()", wait=3.0)
        screenshot("iphone-plan-route")
